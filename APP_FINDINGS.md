...
### INFRA-NEW-tailscale-version-latest — `version: latest` on Tailscale install in 12 composites
**Severity:** P2
...
### INFRA-NEW-restore-xserver-stale-ssh-key — Reusable workflow still passes removed v2.0.0 input
**Severity:** P2
...
### INFRA-NEW-ops-scripts-ref-mutable — `ops_scripts_ref` defaults to `'main'`
**Severity:** P2
...
### INFRA-NEW-provision-curl-pipe-sh — Legacy provision-server installs Docker via `curl | sh`
**Severity:** P2
...
### INFRA-NEW-kuma-prune-injection — `prune` input inlined into shell condition
